...
func (c *RootCmd) Usage() {
	err := executeUsage(os.Stderr, "diff_usage.txt", UsageDataRootCmd{c, false})
	if err != nil {
...
func (c *RootCmd) UsageRecursive() {
	err := executeUsage(os.Stderr, "diff_usage.txt", UsageDataRootCmd{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}
...
func NewRoot(name, version, commit, date string) (*RootCmd, error) {
	...
	{
		subCmd := NewLazyCommand(func() Cmd { return c.NewCompare() })
		c.Commands["compare"] = subCmd
